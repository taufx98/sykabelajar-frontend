...
(function(){
  ...
  function avatarHtml(p){const url=p?.avatar_url||'';return `<span class="feed-avatar">${url?`<img src="${esc(url)}" alt="" loading="lazy">`:esc(U().initials(p?.full_name||'Sykabelajar'))}</span>`;}
  function competitionCard(item){
    const href=window.SYKA_ROUTER.href('/lomba/'+encodeURIComponent(item.slug||''));
    const title=item.title||'Kompetisi';
    const poster=item.poster||item.poster_url||item.image_url||'';
    const transformed=poster?U().cloudinaryTransform(poster,{width:900,height:506,crop:'fill'}):'';
    return `<article class="bolt-card competition-card home-competition-card"><a class="competition-card-media" href="${href}" aria-label="Lihat ${esc(title)}">${transformed?`<img src="${esc(transformed)}" alt="" loading="lazy">`:'<div class="competition-media-placeholder"><span>✦</span><small>Sykabelajar.id</small></div>'}<span class="competition-status-overlay ${U().statusClass(item.status)}">${esc(String(item.status||'').replaceAll('_',' '))}</span></a><div class="competition-card-body"><div class="chip-row"><span class="chip">${esc(item.category||'Kompetisi')}</span></div><h3><a href="${href}">${esc(title)}</a></h3><p>${esc(item.short_description||item.description||'Temukan persyaratan, timeline, hadiah, dan mekanisme kompetisi.')}</p><div class="competition-card-meta"><span><b>Mulai</b>${esc(U().formatDate(item.starts_at||item.startsAt))}</span><span><b>Daftar s/d</b>${esc(U().formatDate(item.registration_ends_at||item.registrationEndsAt))}</span></div><a class="btn btn-primary btn-block btn-sm" href="${href}">Lihat detail <span>→</span></a></div></article>`;
  }
  function postCard(item){
    const href=item.competition_id&&item.slug?window.SYKA_ROUTER.href('/lomba/'+encodeURIComponent(item.slug)):window.SYKA_ROUTER.href('/home');
    const name=item.organizer_name||item.author_name||'Sykabelajar';
    return `<article class="social-post post-update bolt-card"><div class="social-post-head">${avatarHtml({full_name:name,avatar_url:item.organizer_avatar_url||item.author_avatar_url})}<div class="social-post-meta"><strong>${esc(name)}</strong><span>· Update Sykabelajar</span><time>${esc(U().formatDate(item.created_at||item.published_at||new Date().toISOString()))}</time></div><button class="feed-more" type="button" aria-label="Lainnya">•••</button></div><a class="social-post-body" href="${href}"><span class="post-kicker">UPDATE</span><h3>${esc(item.title||item.badge||'Update Sykabelajar')}</h3><p>${esc(item.subtitle||item.description||'Informasi terbaru dari Sykabelajar.')}</p>${item.image_url?`<div class="post-media"><img src="${esc(item.image_url)}" loading="lazy" alt=""></div>`:''}</a><div class="social-post-stats"><button type="button" disabled>♡ ${fmt(item.like_count||0)}</button><button type="button" disabled>◌ ${fmt(item.comment_count||0)}</button><button type="button" data-share-post="${esc(item.title||'Sykabelajar')}">↗ Bagikan</button></div></article>`;
  }
  async function render(root){
    const auth=window.SYKA_STATE.getState().auth;
    root.innerHTML=`<div class="bolt-home-layout"><main class="bolt-home-main"><section class="home-welcome"><div class="eyebrow">DISCOVERY</div><h1>Beranda</h1><p>Temukan lomba, misi, dan prestasi dalam satu ruang belajar.</p></section><section class="home-tabs-card bolt-card"><div class="home-tabs"><button class="active" data-home-tab="lomba" type="button">Lomba</button><button data-home-tab="prestasi" type="button">Prestasi</button></div></section><section class="quick-actions"><a class="quick-action bolt-card" href="${window.SYKA_ROUTER.href('/lomba')}"><span class="quick-action-icon">◈</span><div><strong>Ikuti kompetisi</strong><small>Jelajahi lomba yang sedang dibuka dan pilih yang sesuai.</small></div><span class="quick-arrow">→</span></a><a class="quick-action bolt-card" href="${window.SYKA_ROUTER.href('/tugas')}"><span class="quick-action-icon">✦</span><div><strong>Selesaikan misi harian</strong><small>Dapatkan XP dan Koin Edu dari aktivitas yang tersedia.</small></div><span class="quick-arrow">→</span></a></section><section class="content-section"><div class="section-title"><div><span class="eyebrow">KOMPETISI</span><h2>Kompetisi pilihan</h2><p>Data diambil langsung dari backend Sykabelajar.</p></div><a class="text-link" href="${window.SYKA_ROUTER.href('/lomba')}">Lihat semua →</a></div><div id="home-competition-grid" class="card-grid"></div></section><section class="content-section" id="home-feed-section"><div class="section-title"><div><span class="eyebrow">UPDATE</span><h2>Update terbaru</h2></div></div><div id="home-feed-list" class="home-feed-list"></div></section></main><aside class="home-right-rail"><section class="panel-card home-profile-summary" id="home-profile-summary"><div class="profile-summary-cover"></div><div class="profile-summary-body"><div class="x-avatar-lg" id="home-user-avatar">S</div><strong id="home-user-name">Pengguna</strong><small id="home-user-handle">@user</small><a class="btn btn-secondary btn-sm btn-block" href="${window.SYKA_ROUTER.href('/profile')}">Lihat profil</a></div></section><section class="panel-card"><div class="section-title compact"><div><span class="eyebrow">RINGKASAN</span><h2>Progress kamu</h2></div></div><div id="home-progress-stats" class="home-progress-stats"><div><b>—</b><small>XP</small></div><div><b>—</b><small>Koin Edu</small></div><div><b>—</b><small>Prestasi</small></div></div></section><section class="panel-card"><div class="section-title compact"><div><span class="eyebrow">TRENDING</span><h2>Lomba populer</h2></div><a class="text-link" href="${window.SYKA_ROUTER.href('/lomba')}">Semua</a></div><div id="home-trending" class="trending-list"></div></section></aside></div>`;
    if(auth.user){const p=auth.profile||{};const name=p.full_name||auth.user.user_metadata?.full_name||auth.user.email?.split('@')[0]||'Pengguna';const a=document.getElementById('home-user-avatar');document.getElementById('home-user-name').textContent=name;document.getElementById('home-user-handle').textContent='@'+(p.username||'user');if(p.avatar_url)a.innerHTML=`<img src="${esc(p.avatar_url)}" alt="" loading="lazy">`;}
    const grid=document.getElementById('home-competition-grid');grid.innerHTML=[0,1,2].map(()=>'<div class="competition-card skeleton-card"></div>').join('');
    const feed=document.getElementById('home-feed-list');feed.innerHTML=[0,1,2].map(()=>'<div class="social-post skeleton-post"></div>').join('');
    try{
      const [competitions,updates,tasks,awards]=await Promise.allSettled([
        ...
        auth.user&&window.SYKA_AWARD_SERVICE?.getAwards?window.SYKA_AWARD_SERVICE.getAwards(auth.user.id):Promise.resolve([])
      ]);
      const comps=competitions.status==='fulfilled'?(competitions.value||[]):[];
      grid.innerHTML=comps.length?comps.slice(0,6).map(competitionCard).join(''):window.SYKA_EMPTY.render({title:'Belum ada kompetisi',text:'Kompetisi publik akan muncul di sini setelah dipublikasikan.'});
      document.getElementById('home-trending').innerHTML=comps.slice(0,5).map((c,i)=>`<a class="trend-item" href="${window.SYKA_ROUTER.href('/lomba/'+encodeURIComponent(c.slug||''))}"><span>${i+1}</span><div><strong>${esc(c.title||'Kompetisi')}</strong><small>${esc(String(c.status||'').replaceAll('_',' '))}</small></div></a>`).join('')||'<div class="empty-mini">Belum ada lomba.</div>';
      const p=auth.profile||{};const awardRows=awards.status==='fulfilled'?(awards.value||[]):[];const xp=Number(p.xp_balance??p.xp??p.total_xp??0);const coins=Number(p.edu_coin_balance??p.edu_coins??p.coin_balance??0);document.querySelectorAll('#home-progress-stats b').forEach((el,i)=>el.textContent=U().formatNumber([xp,coins,awardRows.length][i]||0));
      const up=updates.status==='fulfilled'?(updates.value||[]):[];feed.innerHTML=up.length?up.slice(0,6).map(postCard).join(''):(comps.length?comps.slice(0,4).map(c=>`<article class="social-post post-competition bolt-card"><div class="social-post-head">${avatarHtml({full_name:c.organizer_name||'Penyelenggara',avatar_url:c.organizer_avatar_url})}<div class="social-post-meta"><strong>${esc(c.organizer_name||'Penyelenggara')}</strong><span>· Lomba terbaru</span></div></div><a class="social-post-body" href="${window.SYKA_ROUTER.href('/lomba/'+encodeURIComponent(c.slug||''))}"><span class="post-kicker">LOMBA</span><h3>${esc(c.title)}</h3><p>${esc(c.short_description||c.description||'Kompetisi terbaru di Sykabelajar.')}</p></a></article>`).join(''):'<div class="empty-card"><strong>Belum ada update</strong><span>Update resmi akan muncul di sini.</span></div>');
      document.querySelectorAll('[data-home-tab]').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('[data-home-tab]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');if(btn.dataset.homeTab==='prestasi'){feed.innerHTML=awardRows.length?awardRows.slice(0,6).map(a=>`<article class="social-post bolt-card"><div class="social-post-body"><span class="post-kicker">PRESTASI</span><h3>${esc(a.title||a.achievement_title||'Prestasi')}</h3><p>${esc(a.description||'Pencapaian kamu di Sykabelajar.')}</p></div></article>`).join(''):'<div class="empty-card"><strong>Belum ada prestasi</strong><span>Selesaikan kompetisi dan misi untuk membangun rekam prestasi.</span></div>';}else{feed.innerHTML=up.length?up.slice(0,6).map(postCard).join(''):'<div class="empty-card"><strong>Belum ada update</strong><span>Update resmi akan muncul di sini.</span></div>';}}));
      feed.querySelectorAll('[data-share-post]').forEach(b=>b.addEventListener('click',async()=>{try{if(navigator.share)await navigator.share({title:b.dataset.sharePost,url:location.href});else{await navigator.clipboard?.writeText(location.href);window.SYKA_TOAST.show('Link disalin.','success');}}catch(_){}}));
    }catch(e){grid.innerHTML=window.SYKA_EMPTY.render({title:'Data belum tersedia',text:e.message||'Coba lagi nanti.'});feed.innerHTML=window.SYKA_EMPTY.render({title:'Feed belum tersedia',text:e.message||'Coba lagi nanti.'});}
  }
...
/* src/sykabelajar-v2/takeover.js */
 ...
 */
(function(){
  if(window.__SYKA_V2_TAKEOVER__)return;window.__SYKA_V2_TAKEOVER__=true;
  var pathIcon={trophy:'M7 4h10v6a5 5 0 0 1-10 0z M4 6H2v2a4 4 0 0 0 4 4 M20 6h2v2a4 4 0 0 1-4 4 M9 21h6 M12 15v6',spark:'M12 3l1.4 5.6L19 10l-5.6 1.4L12 17l-1.4-5.6L5 10l5.6-1.4z',chart:'M4 19V5 M4 19h16 M8 16v-4 M12 16V8 M16 16v-6'};
  function icon(n){return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="'+(pathIcon[n]||'')+'"></path></svg>'}
  function route(){try{return new URLSearchParams(location.search).get('route')||'/'}catch(e){return '/'}}
  function isLanding(){var r=route();return r==='/'||r===''}
  function supa(){return window.SYKA_SUPABASE&&window.SYKA_SUPABASE.get?window.SYKA_SUPABASE.get():null}
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'})[c]})}
  function initials(v){return String(v||'U').trim().split(/\s+/).slice(0,2).map(function(x){return x[0]||''}).join('').toUpperCase()||'U'}
  function fmt(v){return Number(v||0).toLocaleString('id-ID')}
  function installStyle(){
    if(document.getElementById('syka-v2-production-fixes'))return;
    var s=document.createElement('style');s.id='syka-v2-production-fixes';s.textContent='html.syka-v2-takeover,html.syka-v2-takeover body{margin:0!important;padding:0!important;width:100%!important;min-width:0!important;background:#060b14!important}body.syka-v2-body{overflow-x:hidden!important}body.syka-v2-body>[data-syka-legacy-hidden="true"]{display:none!important}#page-root{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important;min-height:100vh!important;box-sizing:border-box!important}#page-root *{box-sizing:border-box!important}.bolt-layout-controlled.bolt-landing-route #page-root .bolt-side,.bolt-layout-controlled.bolt-landing-route #page-root .v2-sidebar,.bolt-layout-controlled.bolt-landing-route #page-root .bolt-rail,.bolt-layout-controlled.bolt-landing-route #page-root .v2-rail{display:none!important}.bolt-layout-controlled.bolt-landing-route #page-root .bolt-wrap{display:block!important;width:100%!important;max-width:none!important;min-height:100vh!important}.bolt-layout-controlled.bolt-landing-route #page-root .bolt-main{width:100%!important;border:0!important}.bolt-layout-controlled.bolt-landing-route #page-root .bolt-top{display:none!important}.bolt-standalone-landing{min-height:100vh;background:radial-gradient(60rem 60rem at 82% -10%,rgba(16,185,129,.09),transparent 60%),#070b14;color:#f8fafc;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}.bolt-standalone-nav{position:sticky;top:0;z-index:40;border-bottom:1px solid rgba(255,255,255,.06);background:rgba(7,11,20,.9);backdrop-filter:blur(18px)}.bolt-standalone-nav-inner{width:min(1200px,calc(100% - 32px));min-height:70px;margin:auto;display:flex;align-items:center;gap:26px}.bolt-standalone-brand{display:flex;align-items:center;gap:10px;color:#fff;text-decoration:none;font-size:18px;font-weight:800;background:none;border:0;cursor:pointer}.bolt-standalone-mark{width:38px;height:38px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(135deg,#047857,#10b981);box-shadow:0 8px 30px rgba(16,185,129,.14)}.bolt-standalone-brand svg{width:20px;height:20px}.bolt-standalone-gradient{background:linear-gradient(135deg,#a7f3d0,#34d399,#10b981);-webkit-background-clip:text;background-clip:text;color:transparent}.bolt-standalone-links{display:flex;gap:4px;margin-left:auto}.bolt-standalone-links button{border:0;background:none;color:#94a3b8;padding:9px 11px;border-radius:10px;cursor:pointer;font:600 13px Inter,sans-serif}.bolt-standalone-links button:hover{color:#fff;background:rgba(255,255,255,.04)}.bolt-standalone-actions{display:flex;align-items:center;gap:7px}.bolt-standalone-btn{border-radius:11px;padding:11px 15px;font:700 13px Inter,sans-serif;cursor:pointer}.bolt-standalone-btn.primary{background:#059669;color:#fff;border:1px solid #059669}.bolt-standalone-btn.outline{background:transparent;color:#e2e8f0;border:1px solid rgba(255,255,255,.12)}.bolt-standalone-hero{width:min(1200px,calc(100% - 32px));margin:auto;padding:72px 0 50px}.bolt-standalone-hero-grid{display:grid;grid-template-columns:minmax(0,1.08fr) minmax(380px,.92fr);gap:56px;align-items:center}.bolt-standalone-kicker{display:inline-flex;align-items:center;gap:7px;padding:7px 10px;border-radius:999px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.18);color:#6ee7b7;font-size:10px;font-weight:800}.bolt-standalone-kicker svg{width:13px}.bolt-standalone-hero h1{font-family:"Plus Jakarta Sans",Inter,sans-serif;font-size:clamp(44px,5.6vw,72px);line-height:1.02;letter-spacing:-.045em;margin:18px 0}.bolt-standalone-hero p{max-width:650px;color:#94a3b8;font-size:16px;line-height:1.75}.bolt-actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:22px}.bolt-standalone-stats{display:flex;gap:24px;margin-top:30px}.bolt-standalone-stat{display:grid;gap:4px}.bolt-standalone-stat+.bolt-standalone-stat{padding-left:24px;border-left:1px solid rgba(255,255,255,.08)}.bolt-standalone-stat strong{font-size:21px}.bolt-standalone-stat span{font-size:10px;color:#64748b}.bolt-standalone-panel{position:relative;padding:18px;border:1px solid rgba(255,255,255,.07);background:rgba(14,22,38,.86);border-radius:20px;box-shadow:0 24px 70px rgba(0,0,0,.3)}.bolt-standalone-panel-head{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:800;margin-bottom:10px}.bolt-standalone-panel-head svg{width:18px}.bolt-standalone-live{margin-left:auto;color:#6ee7b7;font-size:9px}.bolt-standalone-board-row{display:grid;grid-template-columns:20px 34px minmax(0,1fr) auto;gap:10px;align-items:center;padding:10px 8px;border-radius:10px}.bolt-standalone-board-row:hover{background:rgba(255,255,255,.03)}.bolt-standalone-avatar{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(135deg,#0ea5e9,#10b981);color:#fff;font-size:10px;font-weight:800}.bolt-standalone-board-row strong{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bolt-standalone-board-row small{display:block;color:#64748b;font-size:10px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bolt-standalone-score{font-size:11px;color:#6ee7b7;font-style:normal;font-weight:800}.bolt-standalone-section{width:min(1200px,calc(100% - 32px));margin:auto;padding:28px 0 66px}.bolt-standalone-section-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin-bottom:16px}.bolt-standalone-section-head h2{font-family:"Plus Jakarta Sans",Inter,sans-serif;font-size:27px;letter-spacing:-.03em;margin:0}.bolt-standalone-section-head p{color:#64748b;font-size:12px;margin:5px 0 0}.bolt-standalone-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:15px}.bolt-standalone-card{overflow:hidden;border:1px solid rgba(255,255,255,.07);background:rgba(14,22,38,.78);border-radius:17px}.bolt-standalone-cover{height:165px;background:linear-gradient(135deg,#17213b,#0c3b35);background-size:cover;background-position:center}.bolt-standalone-card-body{padding:16px}.bolt-standalone-chip{display:inline-flex;padding:5px 8px;border-radius:999px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.16);color:#6ee7b7;font-size:10px;font-weight:700}.bolt-standalone-card h3{font-family:"Plus Jakarta Sans",Inter,sans-serif;font-size:15px;line-height:1.4;margin:10px 0 7px}.bolt-standalone-card p{color:#94a3b8;font-size:12px;line-height:1.6;margin:0}.bolt-standalone-card-meta{display:flex;justify-content:space-between;margin-top:13px;color:#64748b;font-size:10px}.bolt-standalone-card-meta strong{color:#6ee7b7}.bolt-standalone-empty{padding:22px;border:1px dashed rgba(255,255,255,.12);border-radius:15px;color:#64748b;text-align:center;font-size:12px;grid-column:1/-1}.bolt-standalone-footer{border-top:1px solid rgba(255,255,255,.06);padding:28px 0;color:#64748b;text-align:center;font-size:11px}@media(max-width:900px){.bolt-standalone-links{display:none}.bolt-standalone-hero-grid{grid-template-columns:1fr}.bolt-standalone-grid{grid-template-columns:1fr 1fr}}@media(max-width:620px){.bolt-standalone-nav-inner{gap:10px}.bolt-standalone-actions .outline{display:none}.bolt-standalone-hero{padding:46px 0 32px}.bolt-standalone-hero h1{font-size:42px}.bolt-standalone-stats{gap:14px}.bolt-standalone-stat+.bolt-standalone-stat{padding-left:14px}.bolt-standalone-grid{grid-template-columns:1fr}.bolt-standalone-section{padding-bottom:45px}}.v2-nav button{min-height:40px!important}.v2-main-body{padding:28px 30px!important}.v2-data-empty{padding:22px 12px;color:var(--v2-muted);font-size:12px;text-align:center;border:1px dashed var(--v2-border2);border-radius:12px}';document.head.appendChild(s)
  }
  function isolate(){var r=document.getElementById('page-root');if(!r)return false;if(r.parentElement!==document.body)document.body.appendChild(r);Array.prototype.forEach.call(document.body.children,function(e){if(e!==r)e.setAttribute('data-syka-legacy-hidden','true')});document.documentElement.classList.add('syka-v2-takeover');document.body.classList.add('syka-v2-body');return true}
  function hideLegacy(){installStyle();isolate();var l=isLanding();document.documentElement.classList.toggle('bolt-layout-controlled',true);document.documentElement.classList.toggle('bolt-landing-route',l);document.documentElement.classList.toggle('bolt-app-route',!l);document.body.classList.toggle('bolt-landing-route',l);document.body.classList.toggle('bolt-app-route',!l);document.querySelectorAll('#app-shell > .sidebar,#app-shell > .main-area > .header,#app-shell > .main-area > #blogger-content,#app-shell > .bottom-nav,#mobile-nav-overlay').forEach(function(e){e.style.setProperty('display','none','important')});var r=document.getElementById('page-root');if(r){r.style.setProperty('display','block','important');r.style.setProperty('width','100%','important');r.style.setProperty('max-width','none','important');r.style.setProperty('margin','0','important');r.style.setProperty('padding','0','important');r.style.setProperty('min-height','100vh','important')}document.querySelectorAll('#page-root .bolt-side,#page-root .v2-sidebar,#page-root .bolt-rail,#page-root .v2-rail').forEach(function(e){if(l)e.style.setProperty('display','none','important');else e.style.removeProperty('display')});document.documentElement.dataset.sykabelajar='v2'}
  function bind(){document.querySelectorAll('[data-syka-route]').forEach(function(el){if(el.__sykaBound)return;el.__sykaBound=true;el.addEventListener('click',function(){var r=el.getAttribute('data-syka-route')||'/';var u=new URL(location.href);u.searchParams.set('route',r);history.pushState({},'',u.pathname+'?'+u.searchParams.toString());render()})})}
  async function landingData(){var competitions=[],leaderboard=[],users=0,certificates=0;try{if(window.SYKA_COMPETITION_SERVICE&&window.SYKA_COMPETITION_SERVICE.list)competitions=await window.SYKA_COMPETITION_SERVICE.list({limit:6,status:'PUBLIC_ONLY'})}catch(e){}try{if(window.SYKA_LEADERBOARD_SERVICE&&window.SYKA_LEADERBOARD_SERVICE.get)leaderboard=await window.SYKA_LEADERBOARD_SERVICE.get({scope:'global',limit:5})}catch(e){}try{var c=supa();if(c){var u=await c.from('profiles').select('id',{count:'exact',head:true});users=u&&u.count||0;var ce=await c.from('certificates').select('id',{count:'exact',head:true});certificates=ce&&ce.count||0}}catch(e){}return{competitions:competitions||[],leaderboard:leaderboard||[],users:users,certificates:certificates}}
  function landingMarkup(d){var comps=d.competitions||[],board=d.leaderboard||[];var bh=board.length?board.map(function(x,i){var n=x.name||x.full_name||x.display_name||x.username||'Peserta';var sc=Number(x.score??x.points??x.xp??x.total_xp??x.total_points??0);return'<div class="bolt-standalone-board-row"><b>'+String(i+1).padStart(2,'0')+'</b><span class="bolt-standalone-avatar">'+esc(initials(n))+'</span><div><strong>'+esc(n)+'</strong><small>'+esc(x.school||x.institution||'Peserta Sykabelajar')+'</small></div><em class="bolt-standalone-score">'+fmt(sc)+'</em></div>'}).join(''):'<div class="bolt-standalone-empty">Belum ada data peringkat dari backend.</div>';var cards=comps.length?comps.map(function(c){var cover=c.poster||c.cover_url||c.image_url||'';var st=cover?' style="background-image:url(\''+esc(cover).replace(/'/g,'%27')+'\')"':'';return'<article class="bolt-standalone-card"><div class="bolt-standalone-cover"'+st+'></div><div class="bolt-standalone-card-body"><span class="bolt-standalone-chip">'+esc(c.category||'Kompetisi')+'</span><h3>'+esc(c.title||'Kompetisi')+'</h3><p>'+esc(c.description||'Ikuti kompetisi dan raih prestasi bersama Sykabelajar.')+'</p><div class="bolt-standalone-card-meta"><strong>Kompetisi Publik</strong><span>'+esc(c.status||'PUBLIC')+'</span></div></div></article>'}).join(''):'<div class="bolt-standalone-empty">Belum ada kompetisi publik yang tersedia.</div>';return'<div class="bolt-standalone-landing"><header class="bolt-standalone-nav"><div class="bolt-standalone-nav-inner"><button class="bolt-standalone-brand" data-syka-route="/"><span class="bolt-standalone-mark">'+icon('trophy')+'</span><span>sykabelajar<span class="bolt-standalone-gradient">.id</span></span></button><nav class="bolt-standalone-links"><button data-syka-route="/lomba">Kompetisi</button><button data-syka-route="/home">Belajar</button><button data-syka-route="/organizer">Organizer</button></nav><div class="bolt-standalone-actions"><button class="bolt-standalone-btn outline" data-syka-route="/login">Masuk</button><button class="bolt-standalone-btn primary" data-syka-route="/register">Daftar Gratis</button></div></div></header><main><section class="bolt-standalone-hero"><div class="bolt-standalone-hero-grid"><div><span class="bolt-standalone-kicker">'+icon('spark')+' Platform Uji Kompetensi Nasional</span><h1>Belajar Jadi Seru,<br><span class="bolt-standalone-gradient">Uji Kompetensi Setiap Hari</span></h1><p>Ikuti uji kompetensi, kumpulkan XP, naikkan peringkat, dan bangun portofolio prestasi yang terhubung langsung dengan ekosistem Sykabelajar.</p><div class="bolt-actions"><button class="bolt-standalone-btn primary" data-syka-route="/register">Mulai Sekarang — Gratis</button><button class="bolt-standalone-btn outline" data-syka-route="/lomba">Jelajahi Kompetisi</button></div><div class="bolt-standalone-stats"><div class="bolt-standalone-stat"><strong>'+((d.users||0)>0?fmt(d.users):'—')+'</strong><span>Peserta</span></div><div class="bolt-standalone-stat"><strong>'+((comps.length||0)>0?fmt(comps.length):'—')+'</strong><span>Kompetisi publik</span></div><div class="bolt-standalone-stat"><strong>'+((d.certificates||0)>0?fmt(d.certificates):'—')+'</strong><span>Sertifikat</span></div></div></div><div class="bolt-standalone-panel"><div class="bolt-standalone-panel-head">'+icon('chart')+' Papan Peringkat <span class="bolt-standalone-live">LIVE</span></div>'+bh+'</div></div></section><section class="bolt-standalone-section"><div class="bolt-standalone-section-head"><div><h2>Kompetisi Unggulan</h2><p>Data diambil langsung dari backend Sykabelajar.</p></div><button class="bolt-standalone-btn outline" data-syka-route="/lomba">Lihat semua</button></div><div class="bolt-standalone-grid">'+cards+'</div></section></main><footer class="bolt-standalone-footer">sykabelajar.id · Platform Uji Kompetensi Nasional Non-Formal</footer></div>'}
  async function renderLanding(){var r=document.getElementById('page-root');if(!r||!isLanding())return false;var d=await landingData();r.innerHTML=landingMarkup(d);bind();hideLegacy();return true}
  async function render(){hideLegacy();if(isLanding()){try{return await renderLanding()}catch(e){console.error('[Sykabelajar] landing render failed',e);return false}}bind();return true}
  async function start(){hideLegacy();try{if(window.SYKA_V2_RUNTIME&&typeof window.SYKA_V2_RUNTIME.start==='function')await window.SYKA_V2_RUNTIME.start();else if(window.SYKA_V2_BOOTSTRAP&&typeof window.SYKA_V2_BOOTSTRAP==='function')await window.SYKA_V2_BOOTSTRAP()}catch(e){console.warn('[Sykabelajar] existing V2 runtime skipped:',e)}await render()}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();window.addEventListener('popstate',function(){setTimeout(render,20)});
  setInterval(function(){var r=document.getElementById('page-root');if(r){hideLegacy();if(isLanding()&&!r.querySelector('.bolt-standalone-landing'))render()}},1500)
}());


/* src/sykabelajar-v2/bolt-ui.js */
...
/* src/sykabelajar-v2/bolt-home.js */
...
/* src/sykabelajar-v2/bolt-polish.js */
...
/* src/sykabelajar-v2/ui-final-polish.js */
...
/* src/sykabelajar-v2/ui-component-polish.js */
...
(function(){
  ...
  #page-root .home-welcome{padding:6px 0 14px!important}.home-welcome h1{margin:5px 0!important;font-size:34px!important;line-height:1.05!important;letter-spacing:-.045em!important}.home-welcome p{margin:0!important;max-width:700px!important;color:var(--muted)!important;font-size:11px!important;line-height:1.65!important}
  #page-root .home-tabs-card{padding:0!important;overflow:hidden!important}.home-tabs{display:flex!important;gap:4px!important;padding:5px!important;border-bottom:1px solid var(--border)!important}.home-tabs button{height:36px!important;min-height:36px!important;padding:0 14px!important;border:0!important;border-radius:10px!important;background:transparent!important;color:var(--muted)!important;font-size:10px!important;font-weight:900!important}.home-tabs button.active{background:var(--brandSoft)!important;color:var(--brand)!important}
  #page-root .quick-actions{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:12px!important;margin-top:12px!important}.quick-action{display:flex!important;align-items:center!important;gap:12px!important;min-height:86px!important;padding:14px!important;border:1px solid var(--border)!important;border-radius:16px!important;background:var(--surface)!important;box-shadow:var(--shadowSm)!important;text-decoration:none!important;transition:.16s ease!important}.quick-action:hover{transform:translateY(-2px)!important;border-color:#7c3aed30!important}.quick-action-icon{width:40px!important;height:40px!important;min-width:40px!important;border-radius:12px!important;background:var(--brandSoft)!important;color:var(--brand)!important;display:grid!important;place-items:center!important;font-size:17px!important}.quick-action>div{min-width:0!important;display:flex!important;flex-direction:column!important;flex:1!important}.quick-action strong{font-size:11px!important;color:var(--text)!important}.quick-action small{margin-top:4px!important;color:var(--muted)!important;font-size:9px!important;line-height:1.5!important}.quick-arrow{font-size:15px!important;color:var(--muted)!important}
  #page-root .section-title.compact{margin-bottom:10px!important}.section-title.compact h2{font-size:18px!important;margin-top:4px!important}
  #page-root .home-profile-summary{overflow:hidden!important;padding:0!important}.profile-summary-cover{height:92px!important;background:radial-gradient(circle at 80% 20%,rgba(124,58,237,.45),transparent 45%),linear-gradient(135deg,#24173c,#0d1a31)!important}.profile-summary-body{padding:0 14px 14px!important;text-align:left!important;margin-top:-18px!important;position:relative!important}.profile-summary-body .x-avatar-lg{width:54px!important;height:54px!important;border:4px solid var(--surface)!important;border-radius:16px!important;overflow:hidden!important;background:var(--brandSoft)!important;color:var(--brand)!important;display:grid!important;place-items:center!important}.profile-summary-body .x-avatar-lg img{width:100%!important;height:100%!important;object-fit:cover!important}.profile-summary-body strong{display:block!important;margin-top:8px!important;font-size:13px!important}.profile-summary-body>small{display:block!important;margin-top:2px!important;color:var(--muted)!important;font-size:9px!important}.profile-summary-body .btn{margin-top:12px!important}
  #page-root .home-progress-stats{display:grid!important;grid-template-columns:repeat(3,1fr)!important;gap:7px!important}.home-progress-stats>div{min-height:67px!important;padding:10px!important;border:1px solid var(--border)!important;border-radius:12px!important;background:var(--surface2)!important}.home-progress-stats b{display:block!important;font-size:16px!important;letter-spacing:-.03em!important}.home-progress-stats small{display:block!important;color:var(--muted)!important;font-size:8px!important;margin-top:3px!important}
  #page-root .home-feed-list{display:grid!important;gap:12px!important}.social-post{overflow:hidden!important}.social-post-head{display:flex!important;align-items:center!important;gap:10px!important;padding:13px 14px 7px!important}.feed-avatar{width:34px!important;height:34px!important;min-width:34px!important;border-radius:11px!important;display:grid!important;place-items:center!important;overflow:hidden!important;background:var(--brandSoft)!important;color:var(--brand)!important;font-size:10px!important;font-weight:900!important}.feed-avatar img{width:100%!important;height:100%!important;object-fit:cover!important}.social-post-meta{min-width:0!important;display:flex!important;flex-direction:column!important;flex:1!important}.social-post-meta strong{font-size:10px!important}.social-post-meta span,.social-post-meta time{font-size:8px!important;color:var(--muted)!important;margin-top:2px!important}.social-post-body{display:block!important;padding:9px 14px 13px!important;text-decoration:none!important}.post-kicker{font-size:8px!important;font-weight:900!important;letter-spacing:.12em!important;color:var(--brand)!important}.social-post-body h3{margin:6px 0!important;font-size:16px!important;line-height:1.35!important;letter-spacing:-.025em!important}.social-post-body p{margin:0!important;font-size:10px!important;line-height:1.6!important;color:var(--muted)!important}.social-post-stats{display:flex!important;gap:8px!important;padding:9px 14px 12px!important;border-top:1px solid var(--border)!important}.social-post-stats button{height:28px!important;min-height:28px!important;padding:0 8px!important;border:0!important;background:transparent!important;color:var(--muted)!important;font-size:8px!important;border-radius:8px!important}.social-post-stats button:hover{background:var(--surface2)!important;color:var(--text)!important}
  #page-root .catalog-toolbar{padding:12px!important;border:1px solid var(--border)!important;background:var(--surface)!important;border-radius:16px!important;box-shadow:var(--shadowSm)!important}.search-wrap{height:40px!important;min-width:300px!important;border-radius:11px!important}.filter-pill{height:32px!important;padding:0 10px!important;display:inline-flex!important;align-items:center!important}
  .competition-card-v46{display:flex!important;flex-direction:column!important;height:100%!important;overflow:hidden!important;background:var(--surface)!important;border:1px solid var(--border)!important;border-radius:18px!important;box-shadow:var(--shadowSm)!important}.competition-media-v46{display:block!important;position:relative!important;aspect-ratio:16 / 9!important;overflow:hidden!important;background:var(--surface2)!important}.competition-media-v46 img{width:100%!important;height:100%!important;object-fit:cover!important;display:block!important}.competition-card-body-v46{display:flex!important;flex:1!important;flex-direction:column!important;padding:14px!important}.competition-card-body-v46 h3{min-height:38px!important;font-size:14px!important;line-height:1.35!important;margin:8px 0 5px!important;letter-spacing:-.02em!important}.competition-card-body-v46 h3 a{color:var(--text)!important}.competition-card-body-v46 p{min-height:31px!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;color:var(--muted)!important;font-size:10px!important;line-height:1.55!important;margin:0!important}.competition-meta-v46{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;margin-top:auto!important;padding:12px 0!important}.competition-meta-v46 span{font-size:8px!important;color:var(--muted)!important;line-height:1.4!important}.competition-meta-v46 b{display:block!important;color:var(--text)!important;font-size:8px!important;margin-bottom:2px!important}.competition-card-body-v46>.btn{margin-top:2px!important}
  #page-root .task-grid-v410{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:12px!important}.task-card-v410{display:flex!important;gap:13px!important;min-height:138px!important;padding:15px!important;border:1px solid var(--border)!important;border-radius:18px!important;background:var(--surface)!important;box-shadow:var(--shadowSm)!important;transition:.16s ease!important}.task-card-v410:hover{transform:translateY(-2px)!important;border-color:#7c3aed30!important}.task-icon-v410{width:42px!important;height:42px!important;min-width:42px!important;border-radius:13px!important;background:var(--brandSoft)!important;color:var(--brand)!important;display:grid!important;place-items:center!important;font-size:18px!important}.task-body-v410{min-width:0!important;display:flex!important;flex-direction:column!important;flex:1!important}.task-meta-v410{display:flex!important;flex-wrap:wrap!important;gap:5px!important}.task-meta-v410 span{height:22px!important;display:inline-flex!important;align-items:center!important;padding:0 7px!important;border-radius:999px!important;background:var(--surface2)!important;color:var(--muted)!important;font-size:7px!important;font-weight:900!important}.task-body-v410 h2{font-size:15px!important;line-height:1.35!important;margin:8px 0 5px!important;letter-spacing:-.02em!important}.task-body-v410 p{font-size:9px!important;line-height:1.55!important;color:var(--muted)!important;margin:0!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important}.task-actions-v410{display:flex!important;gap:7px!important;margin-top:auto!important;padding-top:12px!important}
  #page-root .control-head{display:flex!important;align-items:flex-end!important;justify-content:space-between!important;gap:18px!important;margin-bottom:16px!important}.control-head h1{font-size:30px!important;line-height:1.05!important;margin:5px 0!important;letter-spacing:-.04em!important}.control-head p{font-size:10px!important;color:var(--muted)!important;margin:0!important;line-height:1.5!important}.security-badge{display:inline-flex!important;align-items:center!important;height:28px!important;padding:0 9px!important;border:1px solid var(--border)!important;border-radius:999px!important;background:var(--surface)!important;color:var(--muted)!important;font-size:8px!important;font-weight:900!important}.control-tabs{display:flex!important;gap:4px!important;overflow-x:auto!important;padding:4px!important;margin-bottom:16px!important;border:1px solid var(--border)!important;background:var(--surface)!important;border-radius:14px!important;scrollbar-width:none!important}.control-tab{height:34px!important;min-height:34px!important;padding:0 10px!important;border:0!important;border-radius:9px!important;background:transparent!important;color:var(--muted)!important;font-size:9px!important;font-weight:900!important;white-space:nowrap!important}.control-tab.active{background:var(--brandSoft)!important;color:var(--brand)!important}.control-content,.control-panel{min-width:0!important}.control-table{width:100%!important;overflow:auto!important;border:1px solid var(--border)!important;border-radius:16px!important;background:var(--surface)!important}table{width:100%!important;border-collapse:collapse!important;font-size:9px!important}th{padding:10px!important;text-align:left!important;color:var(--muted)!important;background:var(--surface2)!important;font-size:8px!important;text-transform:uppercase!important;letter-spacing:.06em!important}td{padding:10px!important;border-top:1px solid var(--border)!important;color:var(--text)!important;vertical-align:middle!important}td .btn{height:30px!important;min-height:30px!important;padding:0 9px!important;font-size:8px!important}
  .modal-dialog{width:min(620px,calc(100vw - 30px))!important;max-width:620px!important;border-radius:20px!important}.modal-header{min-height:54px!important;padding:12px 16px!important}.modal-title{font-size:15px!important}.modal-body{padding:14px 16px!important}.auth-tabs{display:flex!important;gap:5px!important;padding:4px!important;background:var(--surface2)!important;border-radius:12px!important;margin-bottom:12px!important}.auth-tab{height:34px!important;min-height:34px!important;flex:1!important;border:0!important;border-radius:9px!important;background:transparent!important;font-size:10px!important;font-weight:900!important;color:var(--muted)!important}.auth-tab.active{background:var(--surface)!important;color:var(--text)!important;box-shadow:var(--shadowSm)!important}.account-type-grid{display:grid!important;grid-template-columns:repeat(3,1fr)!important;gap:7px!important}.account-type-card{min-height:76px!important;padding:10px!important;border-radius:12px!important}.account-type-icon{font-size:18px!important}.account-type-card strong{font-size:9px!important}.account-type-card small{font-size:7px!important;line-height:1.4!important}
  /* compact profile menu / logout */
  #page-root .profile-menu{width:190px!important;padding:6px!important;border-radius:14px!important}
  #page-root .profile-menu button{height:34px!important;min-height:34px!important;padding:0 10px!important;border-radius:9px!important;font-size:9px!important;line-height:1!important}
  #page-root .profile-menu .danger{height:34px!important;min-height:34px!important}
  #page-root #logout-btn{height:34px!important;min-height:34px!important;padding:0 10px!important;width:100%!important}

  @media(max-width:1100px){#page-root .bolt-home-layout{grid-template-columns:1fr!important}#page-root .home-right-rail{grid-template-columns:repeat(3,minmax(0,1fr))!important}#page-root .task-grid-v410{grid-template-columns:1fr 1fr!important}}
  @media(max-width:760px){#page-root .quick-actions,#page-root .task-grid-v410{grid-template-columns:1fr!important}#page-root .home-right-rail{grid-template-columns:1fr!important}#page-root .card-grid{grid-template-columns:1fr!important}#page-root .catalog-toolbar{align-items:stretch!important}#page-root .search-wrap{min-width:0!important;width:100%!important}#page-root .control-head{align-items:flex-start!important;flex-direction:column!important}#page-root .account-type-grid{grid-template-columns:1fr!important}#page-root .task-card-v410{min-height:122px!important}}
  `;
...
/* src/sykabelajar-v2/ui-modal-polish.js */
