...
function publishV2Runtime() {
  const source = resolve(root, 'src/sykabelajar-v2');
  const target = resolve(dist, 'v2');
  if (!existsSync(source)) {
    throw new Error('Missing src/sykabelajar-v2 runtime source.');
  }
  rmSync(target, { recursive: true, force: true });
  cpSync(source, target, {
    recursive: true,
    filter: (from) => !from.endsWith('.md') && !from.includes('/testing/') && !from.includes('\\\\testing\\\\')
  });
}
...
const loaderNewline = String.fromCharCode(10);
const loaderWithV2 = loader
  .replace(
    "  function applyConfig(manifest){",
    ["  function loadModule(url) { return import(url); }", "", "  function applyConfig(manifest){"].join(loaderNewline)
  )
  .replace(
    ".then(function(){ return loadScript((manifest.base || BASE) + '/' + manifest.app); });",
    [
  ...
  .replace(
    ["        window.SYKA_APP.init();", "        return;"].join(loaderNewline),
...
writeFileSync(resolve(dist, 'loader.js'), loaderWithV2);
...
const manifest = {
  ...
  vendor: vendorFile,
  loader: 'loader.js',
  v2: { entry: 'v2/bootstrap.js', mode: 'opt-in' },
  commit,
  generatedAt,
