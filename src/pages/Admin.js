(function() {
    ...
    const fn = window.SYKA_UTILS.formatNumber;
    const tabs = [
        ['dashboard', 'Dashboard'],
        ['users', 'Pengguna'],
    ...
    function shell(tab, title, subtitle) {
        return `<div class="control-head"><div><span class="eyebrow">ADMIN CONTROL PLANE</span><h1>${title}</h1><p>${subtitle}</p></div><div class="control-head-meta"><span class="security-badge">RLS · server authoritative</span></div></div><div class="control-tabs">${tabs.map(([k,l])=>`<button type="button" class="control-tab ${k===tab?'active':''}" data-tab="${k}">${l}</button>`).join('')}</div><div id="control-content"></div>`;
    }
    async function render(root) {
        ...
        if (!auth.user) {
            root.innerHTML = window.SYKA_EMPTY.render({
                ...
                text: 'Panel admin hanya dapat diakses oleh administrator.',
                actionHtml: '<button class="btn btn-primary" id="admin-login">Masuk</button>'
            });
            document.getElementById('admin-login')?.addEventListener('click', () => window.SYKA_APP.openAuth('login', {
                target: '/admin'
            }));
            return;
        }
        if (!auth.roles.includes('admin')) {
            root.innerHTML = window.SYKA_EMPTY.render({
                title: 'Akses ditolak',
                text: 'Akun ini belum memiliki role admin.',
                icon: '⊘'
            });
            return;
        }
        ...
        root.innerHTML = shell(tab, 'Panel Admin', 'Kelola platform, moderasi, kompetisi, transaksi, feature flags, dan audit dari satu control plane.');
        root.querySelectorAll('[data-tab]').forEach(b => b.onclick = () => window.SYKA_ROUTER.navigate('/admin', {
            tab: b.dataset.tab
        }));
        try {
            await renderTab(document.getElementById('control-content'), tab);
        } catch (error) {
            document.getElementById('control-content').innerHTML = window.SYKA_EMPTY.render({
                title: 'Modul gagal dimuat',
                text: error.message || 'Periksa migration/RLS dan coba lagi.',
                actionHtml: '<button class="btn btn-ghost" id="cp-retry">Coba lagi</button>'
            });
            document.getElementById('cp-retry')?.addEventListener('click', () => render(root));
        }
    }
    ...
    async function renderTab(root, tab) {
        const map = {
            dashboard,
            users,
            competitions,
    ...
    async function dashboard(root) {
        const [stats, comps, users, audit, slides] = await Promise.all([svc().platformStats(), svc().listCompetitionsAdmin({
            limit: 200
        }), svc().listUsers({
            limit: 300
        }), svc().listAudit({
            ...
            onOpen: body => {
                ...
                body.querySelector('#comp-form').onsubmit = async e => {
                    ...
                    try {
                        ...
                        feedback.innerHTML = `<div class="inline-error">${esc(error.message||'Gagal menyimpan kompetisi.')}</div>`;
                    }
                };
            }
        });
    ...
    function competitionConfigModal(c) {
        window.SYKA_MODAL.open({
            title: 'Konfigurasi kompetisi',
            wide: true,
            html: `<div class="config-grid"><button class="config-card" id="cfg-level"><span>◫</span><strong>Jenjang & kelas</strong><small>Atur level, allowed grades, points.</small></button><button class="config-card" id="cfg-rules"><span>◌</span><strong>Aturan pendaftaran</strong><small>Twibbon, social proof, quota.</small></button><button class="config-card" id="cfg-reward"><span>✦</span><strong>Reward</strong><small>Juara, poin, emblem, sertifikat.</small></button></div>`,
            onOpen: body => {
                body.querySelector('#cfg-level').onclick = () => levelModal(c.id);
                body.querySelector('#cfg-rules').onclick = () => rulesModal(c.id);
                body.querySelector('#cfg-reward').onclick = () => rewardModal(c.id);
            }
        });
    }
    async function levelModal(compId) {
        const rows = await svc().listLevels(compId);
        window.SYKA_MODAL.open({
            title: 'Jenjang kompetisi',
            wide: true,
            html: `<div class="modal-toolbar"><button class="btn btn-primary btn-sm" id="new-level">+ Level</button></div><div class="stack-list">${rows.map(r=>`<div class="list-card"><strong>${esc(r.label)}</strong><small>${esc(r.code)} · ${esc((r.allowed_grades||[]).join(', '))}</small><span>${r.points} pts</span></div>`).join('')||'<div class="empty-inline">Belum ada level.</div>'}</div>`,
            onOpen: body => body.querySelector('#new-level').onclick = () => {
                window.SYKA_MODAL.open({
                    title: 'Tambah level',
                    html: `<form id="lf" class="form-card"><label>Kode<input id="code" required placeholder="SD6"></label><label>Label<input id="label" required placeholder="Kelas 6 SD"></label><label>Allowed grades<textarea id="grades">SD6</textarea></label><label>Points<input id="points" type="number" value="0"></label><button class="btn btn-primary">Simpan</button></form>`,
                    onOpen: b => b.querySelector('#lf').onsubmit = async e => {
                        e.preventDefault();
                        try {
                            await svc().saveLevel({
                                competition_id: compId,
                                code: b.querySelector('#code').value.trim(),
    ...
    async function rulesModal(compId) {
        const r = await svc().getRegistrationRules(compId) || {};
        window.SYKA_MODAL.open({
            ...
            onOpen: b => b.querySelector('#rf').onsubmit = async e => {
                e.preventDefault();
                try {
    ...
    async function questions(root) {
        const banks = await svc().listQuestionBanks();
        root.innerHTML = `<div class="toolbar"><div><h2>Question Builder</h2><p>Bank soal, moderation, answer key—tetap dibatasi RLS.</p></div><button class="btn btn-primary" id="new-bank">+ Bank soal</button></div><div class="data-table">${banks.map(b=>`<div class="data-row"><div><strong>${esc(b.name)}</strong><small>${esc(b.description||'—')}</small></div><span class="status-pill ${window.SYKA_UTILS.statusClass(b.status||'DRAFT')}">${esc(b.status||'DRAFT')}</span></div>`).join('')||window.SYKA_EMPTY.render({title:'Belum ada bank soal',text:'Buat bank soal untuk mulai menyusun question set.'})}</div>`;
        document.getElementById('new-bank').onclick = () => window.SYKA_MODAL.open({
            title: 'Bank soal baru',
            html: `<form id="bf" class="form-card"><label>Nama bank soal<input id="name" required></label><label>Deskripsi<textarea id="desc"></textarea></label><label>Status<select id="status"><option>DRAFT</option><option>REVIEW</option><option>PUBLISHED</option></select></label><button class="btn btn-primary">Simpan</button></form>`,
            ...
            onOpen: b => {
                ...
                b.querySelector('#twf').onsubmit = async e => {
                    ...
                    try {
                        ...
                        if (!url) {
                            feedback.innerHTML = '<div class="inline-error">Upload template terlebih dahulu.</div>';
                            return;
                        }
                        await svc().saveTwibbonTemplate({
                            ...
                            is_active: true,
                            config: {
                                width: Number(b.querySelector('#width').value) || null,
                                height: Number(b.querySelector('#height').value) || null,
                                version: b.querySelector('#version').value || null,
                                resource_type: b.querySelector('#resource').value || 'image'
                            }
                        });
                        window.SYKA_MODAL.close();
                        ...
                        feedback.innerHTML = `<div class="inline-error">${esc(error.message||'Template gagal disimpan.')}</div>`;
                    }
                };
            }
        });
    ...
    async function orders(root) {
        ...
        filter.onchange = () => root.querySelectorAll('[data-order-status]').forEach(r => r.style.display = !filter.value || r.dataset.orderStatus === filter.value ? 'grid' : 'none');
    ...
    const PLAN_FEATURES = [{
            key: 'competition_create',
            label: 'Buat kompetisi',
            help: 'Jumlah kompetisi yang dapat dibuat',
            kind: 'limit',
            defaultLimit: 1
        },
        {
            ...
            label: 'Batas peserta',
            help: 'Maksimum peserta per kompetisi',
            kind: 'limit',
            defaultLimit: 50
        },
        {
            key: 'question_bank',
            label: 'Bank soal',
            help: 'Jumlah bank soal yang dapat dikelola',
            kind: 'limit',
            defaultLimit: 1
        },
        {
            key: 'question_limit',
            label: 'Batas soal',
            help: 'Maksimum soal yang tersedia',
            kind: 'limit',
            defaultLimit: 100
        },
        {
            ...
            help: 'Essay / file dapat dinilai manual',
            kind: 'toggle'
        },
        {
            key: 'certificate',
            label: 'Sertifikat',
            help: 'Generate dan publish certificate',
            kind: 'toggle'
        },
        {
            key: 'twibbon',
            label: 'Twibbon',
            help: 'Template dan review twibbon',
            kind: 'toggle'
        },
        {
            key: 'analytics',
            label: 'Analytics',
            help: 'Insight kompetisi dan peserta',
            kind: 'toggle'
        },
        {
            key: 'advanced_reports',
            label: 'Laporan lanjutan',
            help: 'Export dan laporan operasional lebih lengkap',
            kind: 'toggle'
        },
        {
            key: 'bulk_notification',
            label: 'Notifikasi massal',
            help: 'Kirim pengumuman ke banyak peserta',
            kind: 'toggle'
        },
        {
            key: 'custom_branding',
            label: 'Branding khusus',
            help: 'Logo, warna, dan identitas organizer',
            kind: 'toggle'
        },
        {
            key: 'priority_support',
            label: 'Priority support',
            help: 'Dukungan prioritas',
            kind: 'toggle'
        }
    ];
    async function plans(root) {
        ...
        const fallback = {
            FREE: {
                name: 'Free',
                badge: 'Mulai',
                description: 'Untuk penyelenggara yang baru mulai.',
                monthly_price: 0,
                yearly_price: 0,
                is_active: true
            },
            PREMIUM: {
                name: 'Premium',
                badge: 'Populer',
                description: 'Untuk penyelenggara aktif.',
            ...
            PRO: {
                ...
                description: 'Untuk organizer skala besar.',
                monthly_price: 349000,
                yearly_price: 3490000,
                is_active: true
            }
        };
        const byCode = Object.fromEntries((catalog || []).map(p => [p.plan_code, p]));
        const entitlementByPlan = {};
        (ents || []).forEach(e => ((entitlementByPlan[e.plan_code] ??= []).push(e)));
        ...
        const planCard = (code) => {
            ...
            const count = (entitlementByPlan[code] || []).length;
            return `<button type="button" class="plan-preset-card ${selected===code?'selected':''}" data-plan-preset="${code}"><div class="plan-preset-top"><span class="plan-badge ${code.toLowerCase()}">${esc(p.badge||code)}</span><span class="chip">${p.is_active?'AKTIF':'NONAKTIF'}</span></div><h3>${esc(p.name||code)}</h3><p>${esc(p.description||'')}</p><strong>${p.monthly_price?money(p.monthly_price)+'/bulan':'Gratis'}</strong><small>${count} capability aktif</small></button>`;
        };
        const renderEditor = () => {
            const p = byCode[selected] || {
                plan_code: selected,
                ...fallback[selected]
            };
            const current = Object.fromEntries((entitlementByPlan[selected] || []).map(e => [e.capability, e]));
            const cards = PLAN_FEATURES.map(f => {
                const e = current[f.key];
                const checked = !!e;
                return `<label class="plan-feature-row"><span class="check-wrap"><input type="checkbox" data-cap="${f.key}" ${checked?'checked':''}></span><span class="plan-feature-copy"><strong>${esc(f.label)}</strong><small>${esc(f.help)}</small></span>${f.kind==='limit'?`<span class="plan-limit"><input type="number" min="0" step="1" data-limit="${f.key}" value="${e?.limit_value??f.defaultLimit??''}" ${checked?'':'disabled'} placeholder="∞"></span>`:`<span class="plan-enabled-dot">${checked?'ON':'OFF'}</span>`}</label>`
            }).join('');
            const editor = document.getElementById('plan-editor');
            editor.innerHTML = `<div class="plan-editor-head"><div><span class="eyebrow">PLAN BUILDER</span><h2>${esc(p.name||selected)}</h2><p>Pilih kemampuan dengan checkbox. Limit hanya perlu diisi pada fitur kuota.</p></div><div class="plan-editor-price"><label>Harga / bulan<input id="plan-monthly" type="number" min="0" value="${Number(p.monthly_price)||0}"></label><label>Harga / tahun<input id="plan-yearly" type="number" min="0" value="${Number(p.yearly_price)||0}"></label></div></div><div class="form-grid-2 plan-meta-grid"><label>Nama paket<input id="plan-name" value="${esc(p.name||selected)}"></label><label>Badge<input id="plan-badge" value="${esc(p.badge||'')}"></label><label class="span-2">Deskripsi<textarea id="plan-description" rows="2">${esc(p.description||'')}</textarea></label></div><div class="plan-feature-list">${cards}</div><div class="plan-editor-actions"><label class="switch-line"><input id="plan-active" type="checkbox" ${p.is_active!==false?'checked':''}><span>Plan aktif dan boleh dipakai</span></label><button class="btn btn-primary" id="save-plan-bundle">Simpan paket ${esc(selected)}</button></div>`;
            editor.querySelectorAll('[data-cap]').forEach(cb => cb.onchange = () => {
                const limit = editor.querySelector(`[data-limit="${cb.dataset.cap}"]`);
                if (limit) limit.disabled = !cb.checked;
            });
            editor.querySelector('#save-plan-bundle').onclick = async () => {
                const ent = PLAN_FEATURES.flatMap(f => {
                    const cb = editor.querySelector(`[data-cap="${f.key}"]`);
                    if (!cb?.checked) return [];
                    const limitEl = editor.querySelector(`[data-limit="${f.key}"]`);
                    return [{
                        capability: f.key,
                        limit_value: f.kind === 'limit' ? (limitEl.value === '' ? null : Number(limitEl.value)) : 1,
                        config: {}
                    }];
                });
                const btn = editor.querySelector('#save-plan-bundle');
                btn.disabled = true;
                btn.innerHTML = '<span class="spinner"></span> Menyimpan…';
                try {
                    await svc().savePlanBundle({
                        plan_code: selected,
                        name: editor.querySelector('#plan-name').value.trim(),
                        badge: editor.querySelector('#plan-badge').value.trim(),
                        description: editor.querySelector('#plan-description').value.trim(),
                        monthly_price: editor.querySelector('#plan-monthly').value,
                        yearly_price: editor.querySelector('#plan-yearly').value,
                        is_active: editor.querySelector('#plan-active').checked,
                        entitlements: ent
                    });
                    window.SYKA_TOAST.show(`Paket ${selected} berhasil diperbarui.`, 'success');
                    window.SYKA_ROUTER.refresh();
                } catch (error) {
                    window.SYKA_TOAST.show(error.message || 'Paket gagal disimpan.', 'error');
                    btn.disabled = false;
                    btn.textContent = `Simpan paket ${selected}`;
                }
            };
        };
        ...
        root.querySelectorAll('[data-plan-preset]').forEach(b => b.onclick = () => {
            selected = b.dataset.planPreset;
            root.querySelectorAll('[data-plan-preset]').forEach(x => x.classList.toggle('selected', x === b));
            renderEditor();
        });
        renderEditor();
        ...
        async function renderAssignments() {
            try {
                ...
                const rows = await Promise.all(organizers.map(async org => ({
                    org,
                    plan: await svc().listActiveOrganizerPlan(org.id).catch(() => null)
                })));
                assignmentSection.innerHTML = `<div class="panel-head"><div><span class="eyebrow">WORKSPACE ASSIGNMENT</span><h3>Plan aktif per penyelenggara</h3><p>Catalog menentukan paket yang tersedia. Di sini Admin menentukan paket yang benar-benar aktif untuk workspace.</p></div></div><div class="assignment-grid">${rows.map(({org,plan})=>` < div class = "assignment-row" > < div > < strong > $ {
                    esc(org.name)
                } < /strong><small>${esc(org.slug||'')} · ${plan?.plan_code?`Paket ${esc(plan.plan_code)}`:'Belum memilih paket'}</small > < /div><div class="assignment-actions"><select data-assign-org="${esc(org.id)}"><option value="">Pilih paket…</option > $ {
                    ['FREE', 'PREMIUM', 'PRO'].map(code => `<option value="${code}" ${plan?.plan_code===code?'selected':''}>${code}</option>`).join('')
                } < /select><button class="btn btn-secondary btn-sm" data-assign-save="${esc(org.id)}">Simpan</button > < /div></div > `).join('')||'<div class="empty-inline">Belum ada workspace organizer.</div>'}</div>`;
                assignmentSection.querySelectorAll('[data-assign-save]').forEach(btn => btn.onclick = async () => {
                    ...
                    if (!code) {
                        window.SYKA_TOAST.show('Pilih paket terlebih dahulu.', 'warning');
                        return;
                    }
                    btn.disabled = true;
                    btn.textContent = 'Menyimpan…';
                    try {
                        await svc().assignOrganizerPlan(orgId, code);
                        window.SYKA_TOAST.show(`Paket ${code} diaktifkan untuk workspace.`, 'success');
                        await renderAssignments();
                    } catch (error) {
                        window.SYKA_TOAST.show(error.message || 'Gagal menetapkan paket.', 'error');
                        btn.disabled = false;
                        btn.textContent = 'Simpan';
                    }
                });
            } catch (error) {
                assignmentSection.innerHTML = `<div class="inline-error">${esc(error.message||'Assignment plan gagal dimuat.')}</div>`;
            }
        }
        renderAssignments();
    }
    ...
    async function monetization(root) {
        const products = await svc().listCommerceProducts({
            admin: true
        });
        const benefits = {};
        await Promise.all(products.map(async p => {
            benefits[p.id] = await svc().listCommerceBenefits(p.id);
        }));
        ...
        const typeLabels = {
            EDU_COIN_TOPUP: 'Koin Edu',
            FEATURE_UNLOCK: 'Fitur',
        ...
        root.querySelectorAll('[data-toggle-product]').forEach(b => b.onclick = async () => {
            const product = products.find(p => p.id === b.dataset.toggleProduct);
            try {
                await svc().saveCommerceProduct({
                    ...product,
                    is_active: b.dataset.active === 'true'
                }, product.id);
                window.SYKA_TOAST.show('Status produk diperbarui.', 'success');
                render(root);
            } catch (error) {
                window.SYKA_TOAST.show(error.message, 'error');
            }
                ...
                onOpen: b => {
                    ...
                    b.querySelector('#product-form').onsubmit = async e => {
                        ...
                        try {
                            ...
                            await svc().replaceCommerceBenefits(saved.id, benefits);
                            window.SYKA_MODAL.close();
                            window.SYKA_TOAST.show('Produk tersimpan.', 'success');
                            render(root);
                        } catch (error) {
                            b.insertAdjacentHTML('beforeend', `<div class="inline-error">${esc(error.message||'Produk gagal disimpan.')}</div>`);
                        }
                    };
                }
            });
            ...
            onOpen: b => {
                ...
                b.querySelector('#setting-form').onsubmit = async e => {
                    ...
                    try {
                        ...
                        feedback.innerHTML = `<div class="inline-error">${esc(error.message||'Setting tidak valid.')}</div>`;
                    }
                };
            }
        });
            ...
            onOpen: b => {
                ...
                b.querySelector('#promo-upload').onclick = async () => {
                    try {
                        ...
                        b.querySelector('#slide-feedback').innerHTML = `<div class="inline-error">${esc(e.message||'Upload gagal.')}</div>`;
                    }
                };
                b.querySelector('#slide-form').onsubmit = async e => {
                    ...
                    try {
                        ...
                        await svc().saveSlide({
                            ...
                            sort_order: 0,
                            config: {
                                public_id: b.querySelector('#pid').value || null,
                                width: Number(b.querySelector('#w').value) || null,
                                height: Number(b.querySelector('#h').value) || null,
                                version: b.querySelector('#v').value || null,
                                resource_type: b.querySelector('#r').value || 'image'
                            }
                        });
                        window.SYKA_MODAL.close();
                        ...
                        feedback.innerHTML = `<div class="inline-error">${esc(error.message||'Promo slide gagal disimpan.')}</div>`;
                    }
                };
            }
        });
